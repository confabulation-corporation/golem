...
//   console.log("TARGET:", target, golemTarget);
...
//     // TODO 3: Default, what does it do?

//     const golemTargetKeys: string[] = Object.keys(golemTarget);
//     // console.log(golemTargetKeys);
...
//       console.log(contextOfCurrentTarget);
      
//       // This gets the previous context. Array contextOfCurrentTarget is saving the context as we loop through the targets
//       const previousContext: string | undefined = contextOfCurrentTarget[0] || '';
//       console.log("pC", previousContext);
...
//         const placeholder = p1.trim(); // Remove the curly braces from the placeholder
...
//       if (!taskGenerationResponse) {
//         context.set(target, `Default value for ${target}`);
//         // break;
//       }
      
//       contextOfCurrentTarget.length = 0; //clear the previous context
//       contextOfCurrentTarget.push(taskGenerationResponse); //append the new context to be used in the next iteration
//       console.log("Cccc", contextOfCurrentTarget);

//       allOutputs[golemTargetKeys[i]] = taskGenerationResponse;

//       const lines: string[] = taskGenerationResponse.split('\n');
...
async function executeAIChat(target: string, golemFile: GolemFile, golemFilePath: string, context: Map<string, any>): Promise<void> {
  ...
  if (model === 'cat') {
    ...
    context.set(target, concatenatedOutput);
      ...
      const golemTargetKeys: string[] = Object.keys(golemTarget);
      // console.log("gTK length: ", golemTargetKeys.length);

      // TODO 1: Check if prompt asks for additional targets.
      // TODO 2: Check if targets have other dependencies.
      // TODO 3: Default not good
